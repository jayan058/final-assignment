...
class Tower {
    ...
    launchProjectile() {
        ...
        if (currentTime - this.lastShootTime > this.shootInterval && towers.indexOf(this) === Tower.shooterIndex) {
            let  nearestEnemy = this.findNearestEnemy();
            if (nearestEnemy) {
                const peakHeight = 100; 
                const projectile = new ProjectileForTowers(this.x + 60, this.y, nearestEnemy.x, nearestEnemy.y, peakHeight);
                projectilesfortowers.push(projectile);
                this.lastShootTime = currentTime;
            }
        }
        console.log(`Tower at (${this.x}, ${this.y}) shoot interval: ${this.shootInterval}`);
    }

    static updateShooter() {
        const currentTime = Date.now();
        if (currentTime - Tower.lastShooterChangeTime > Tower.shooterChangeInterval) {
    ...
    drawHealthBar() {
        ...
        ctx1.fillRect(healthBarX, healthBarY, healthBarWidth * healthPercentage, healthBarHeight);
    ...
    checkCollisionWithEnemies() {
        ...
        console.log(`Tower at (${this.x}, ${this.y}) is colliding: ${this.isColliding}, health: ${this.health}`);
    }
}
